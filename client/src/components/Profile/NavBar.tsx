...
const NavBar = () => {
  ...
  let subProfilePage = pathname.split('/')?.[2];
  if (subProfilePage === undefined) subProfilePage = 'profile';

  console.log('pathname: ', pathname);
  console.log('subProfilePage: ', subProfilePage);

  function linkClasses(type = 'profile') {
    let classes = 'flex gap-1 px-6 py-2 rounded-full';

    if (type === subProfilePage) {
      classes += ' bg-primary-100 text-white';
    } else {
      classes += ' bg-gray-100';
    }
    return classes;
